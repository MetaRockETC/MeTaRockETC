...
class ExperimentBuilder(object):
    ...
    def build_loss_summary_string(self, summary_losses):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "accuracy" in key or "precision" in key or "recall" in key or "f1-score" in key:
                value = float(value)
    ...
    def train_iteration(self, train_sample, sample_idx, epoch_idx, total_losses, current_iter, pbar_train):
        ...
        pbar_train.update(1)
        pbar_train.set_description("training phase {} -> {}\n".format(self.epoch, train_output_update))
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        mc = classification_report(per_batch_targets.reshape(-1), per_batch_max.reshape(-1), digits=4)
