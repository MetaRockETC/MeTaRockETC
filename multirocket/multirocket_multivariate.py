...
@njit("float32[:](float64[:,:,:],int32[:],int32[:],int32[:],int32[:],float32[:])",
      fastmath=True, parallel=False, cache=False)
...
@njit(
    ...
    fastmath=True, parallel=True, cache=False)
...
class MultiRocket:
    ...
    def fit(self, x_train, device):
        x_train = np.array(x_train.to('cpu'), dtype=np.float64)
        if self.verbose > 1:
        ...
        return torch.tensor(x_train_transform, dtype=torch.float32).to(device)
