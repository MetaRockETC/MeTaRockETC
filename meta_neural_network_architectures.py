...
class MetaMLPLayer(nn.Module):
    def __init__(self, input_shape, num_filters, use_bias):
        ...
        :param use_bias: Whether to use biases or not.
        """
        super(MetaMLPLayer, self).__init__()
        self.input_shape = input_shape
        self.use_bias = use_bias
        self.weights = nn.Parameter(torch.ones(num_filters, self.input_shape))
        nn.init.xavier_uniform_(self.weights)
        if self.use_bias:
            self.bias = nn.Parameter(torch.zeros(num_filters))
...
class MetaMLPNormLayerReLU(nn.Module):
    def __init__(self, input_shape, use_bias, args, normalization=True, meta_layer=True, no_bn_learnable_params=False, device=None):
        super(MetaMLPNormLayerReLU, self).__init__()
        self.normalization = normalization
        self.use_per_step_bn_statistics = args.per_step_bn_statistics
        self.input_shape = input_shape
        self.args = args
    ...
    def build_block(self):
        ...
        x = torch.zeros(self.input_shape).to(self.device)

        out = x

        self.linear = MetaMLPLayer(input_shape=out.shape[1], num_filters=self.num_filters, use_bias=self.use_bias).to(self.device)
        ...
        if self.normalization:
            if self.args.norm_layer == "batch_norm":
                ...
                self.norm_layer = MetaLayerNormLayer(input_feature_shape=out.shape[1:]).to(self.device)
...
class MLPReLUNormNetwork(nn.Module):
    def __init__(self, im_shape, num_output_classes, args, device, meta_classifier=True):
        """
        Builds a multilayer convolutional network. It also provides functionality for passing external parameters to be
        used at inference time. Enables inner loop optimization readily.
        ...
        be enabled.
        """
        super(MLPReLUNormNetwork, self).__init__()
        # b, c, self.h, self.w = im_shape
        self.device = device
        self.total_layers = 0
        self.args = args
    ...
    def build_network(self):
        ...
        print("build network...")
        x = torch.zeros(self.input_shape).to(self.device)
        out = self.rocket.fit(x, self.device)

        self.layer_dict = nn.ModuleDict()
        for i in range(self.num_stages):
            self.layer_dict['mlp{}'.format(i)] = MetaMLPNormLayerReLU(input_shape=out.shape,
                                                                        use_bias=True, args=self.args,
                                                                        normalization=True,
                                                                        meta_layer=self.meta_classifier,
                                                                        no_bn_learnable_params=False,
                                                                        device=self.device).to(self.device)
            out = self.layer_dict['mlp{}'.format(i)](out, training=True, num_step=0)
        
        self.encoder_features_shape = list(out.shape)
        self.out = out.view(out.shape[0], -1)

        self.layer_dict['linear'] = MetaMLPLayer(input_shape=np.prod(self.out.shape[1:]),
                                                    num_filters=self.num_output_classes, use_bias=True).to(self.device)

        out = self.layer_dict['linear'](out)
        print("MLPNetwork build", out.shape)
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False):
        ...
        with torch.no_grad():
            ...
            out = self.rocket.fit(x, self.device)
            e_time = time.time()
            print('Feature Extraction spend {}s'.format(e_time-s_time))
            self.extraction_time+=e_time-s_time
        s_time = time.time()
        for i in range(self.num_stages):
            if 'mlp{}'.format(i) not in param_dict:
                params_tmp = None
            else:
                params_tmp = param_dict['mlp{}'.format(i)]
            out = self.layer_dict['mlp{}'.format(i)](out, params=params_tmp, training=training,
                                                      backup_running_statistics=backup_running_statistics,
                                                      num_step=num_step)
        #     if self.args.max_pooling:
        #         out = F.max_pool2d(input=out, kernel_size=(2, 2), stride=2, padding=0)
    ...
    def restore_backup_stats(self):
        ...
        """
        for i in range(self.num_stages):
            self.layer_dict['mlp{}'.format(i)].restore_backup_stats()
    ...
    def reset_clf(self, num_output_classes, use_bias=True):
        self.layer_dict['linear'] = MetaMLPLayer(input_shape=np.prod(self.out.shape[1:]),
                                                    num_filters=num_output_classes, use_bias=use_bias).to(self.device)