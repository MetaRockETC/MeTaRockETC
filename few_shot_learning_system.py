...
from meta_neural_network_architectures import VGGReLUNormNetwork, ResNet12, MetaLossNetwork, LossAdapter, MLPReLUNormNetwork
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        if self.args.backbone == 'ResNet12':
            ...
            self.classifier = MLPReLUNormNetwork(im_shape=self.im_shape, num_output_classes=self.args.
                                             num_classes_per_set,
                                             args=args, device=device, meta_classifier=True).to(device=self.device)

        self.task_learning_rate = args.init_inner_loop_learning_rate
        self.init_optimizer(args, device)
    ...
    def init_optimizer(self, args, device):
        self.inner_loop_optimizer = LSLRGradientDescentLearningRule(args=args,
                                                                    device=self.device,
                                                                    init_learning_rate=self.task_learning_rate,
                                                                    init_weight_decay=self.args.init_inner_loop_weight_decay,
    ...
    def get_across_task_loss_metrics(self, total_losses, total_accuracies, total_precision, total_recall, total_f1):
        losses = dict()

        losses['loss'] = torch.stack(total_losses)
        losses['accuracy'] = total_accuracies
        losses['precision'] = total_precision
        losses['recall'] = total_recall
        losses['f1-score'] = total_f1
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase):
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in \
            ...
            c, n, d, f = x_target_set_task.shape

            x_support_set_task = x_support_set_task.view(-1, d, f)
            y_support_set_task = y_support_set_task.view(-1)
            x_target_set_task = x_target_set_task.view(-1, d, f)
            ...
            y_true.extend(y_target_set_task.cpu().tolist())
    ...
    def run_train_iter(self, data_batch, epoch):
        ...
        for nt in range(self.args.batch_size):
            ...
            if stacked_loss is None:
                ...
                stacked_precision = np.array([losses['precision']])
                stacked_recall = np.array([losses['recall']])
                stacked_f1 = np.array([losses['f1-score']])
            else:
                stacked_loss = torch.cat((stacked_loss, losses['loss'].detach()), 0)
                stacked_acc = np.concatenate((stacked_acc, losses['accuracy']), 0)
                stacked_precision = np.concatenate((stacked_precision, np.array([losses['precision']])), 0)
                stacked_recall = np.concatenate((stacked_recall, np.array([losses['recall']])), 0)
                stacked_f1 = np.concatenate((stacked_f1, np.array([losses['f1-score']])), 0)
        ...
        losses['f1-score'] = np.mean(stacked_f1)
